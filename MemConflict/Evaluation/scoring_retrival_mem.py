...
def build_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Score Retrival-Mem evaluation results.")
    parser.add_argument(
        "--input_file",
        type=str,
        default=os.path.join(CURRENT_DIR, "Results", "retrival_mem_results.jsonl"),
        help="Input JSONL file containing Retrival-Mem results.",
    )
    parser.add_argument(
        "--output_file",
        type=str,
        default=os.path.join(CURRENT_DIR, "Scores", "retrival_mem_eval_scores.jsonl"),
        help="Output JSONL file for Retrival-Mem evaluation scores.",
    )
    parser.add_argument(
        "--output_perfect_file",
        type=str,
        default=os.path.join(CURRENT_DIR, "Scores", "retrival_mem_eval_scores.json"),
        help="Output JSON file for Retrival-Mem evaluation scores.",
    )
    parser.add_argument(
        "--prediction_fields",
        type=str,
